...
import { FirebasemanualesModel } from './firebasemanuales/firebasemanuales.model.model';
...
export class FirebaseCrudService {
  ...
  public searchManualesbyName(lower: number, upper: number): Observable<Array<FirebasemanualesModel>> {


    const filteredDataSource: Observable<Array<FirebasemanualesModel>> = collectionData<FirebasemanualesModel>(
      query<FirebasemanualesModel>(
        collection(this.firestore, 'uniprotec/user/users') as CollectionReference<FirebasemanualesModel>,
        where('name', '==', name)
      ), { idField: 'id' }
    )
      .pipe(
        map((users: Array<FirebasemanualesModel>) => {
          return users.map((user: FirebasemanualesModel) => {
            //const age = this.calcUserAge(user.name);

            return { ...user } as FirebasemanualesModel;
          });
        })
      );
